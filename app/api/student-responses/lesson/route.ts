...
export async function POST(request: Request) {
  ...
  if (!body.totalSections || body.totalSections < 1) return NextResponse.json({ error: 'Missing lesson section count.' }, { status: 400 });
  ...
  const responsePayload = { answers, sectionHeadings: body.sectionHeadings ?? [], currentSection: body.currentSection, totalSections: body.totalSections, completedAnswers, completionPercentage, finished: body.finished };
  ...
  if (existing?.id) {
    const { error } = await supabase.from('student_responses').update({ response_type: 'lesson_content', response_json: responsePayload, score: completedAnswers, status, last_saved_at: now, submitted_at: status === 'complete' ? now : null }).eq('id', existing.id);
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    return NextResponse.json({ status: 'updated', savedAt: now, ...responsePayload });
  }

  const { error } = await supabase.from('student_responses').insert({ student_id: access.legacyUserId, assignment_id: DEMO_ASSIGNMENT_ID, activity_id: body.activityId, response_type: 'lesson_content', response_json: responsePayload, score: completedAnswers, status, started_at: now, last_saved_at: now, submitted_at: status === 'complete' ? now : null });
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
