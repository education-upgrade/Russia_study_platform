...
const LESSON_TITLE = 'Why did Alexander II believe Russia needed reform?';
const PATHWAY_SLUG = 'alexander-ii-reform';
...
const activityRouteMap: Record<string, string> = {
  lesson_content: '/student/lesson/alexander-ii-reform/lesson',
  flashcards: '/student/lesson/alexander-ii-reform/flashcards',
  quiz: '/student/lesson/alexander-ii-reform/quiz',
  peel_response: '/student/lesson/alexander-ii-reform/peel',
  confidence_exit_ticket: '/student/lesson/alexander-ii-reform/confidence',
};
...
function getActivityStatus(activity: Activity, response: StudentResponse | undefined, hasAnyEvidence: boolean): ActivityStatus {
  ...
  if (!response) {
    return { label: 'To do', detail: 'No saved evidence yet.', tone: 'neutral', isComplete: false, isTrackable: true };
  }
  ...
  if (activity.activity_type === 'quiz') {
    ...
    return { label: isComplete ? 'Done' : 'Started', detail: `${response.score ?? '-'}/${json.maxScore ?? '?'}${percentage}`, tone: isComplete ? 'complete' : 'started', isComplete, isTrackable: true };
  }
  ...
  if (activity.activity_type === 'flashcards') {
    ...
    return { label: isComplete ? 'Done' : 'Started', detail: `${ratedCount}/${totalCards} rated${revisitCount ? ` · ${revisitCount} revisit` : ''}`, tone: isComplete ? (revisitCount > 0 ? 'warning' : 'complete') : 'started', isComplete, isTrackable: true };
  }

  if (activity.activity_type === 'peel_response') {
    const wordCount = json.wordCount ?? 0;
    const isComplete = response.status === 'complete' || response.status === 'submitted';
    return { label: isComplete ? 'Done' : 'Started', detail: `${wordCount} words`, tone: wordCount < 40 ? 'warning' : 'complete', isComplete, isTrackable: true };
  }

  if (activity.activity_type === 'confidence_exit_ticket') {
    const confidence = json.confidence ?? response.score ?? '-';
    const isComplete = response.status === 'complete' || response.status === 'submitted';
    return { label: isComplete ? 'Done' : 'Started', detail: `Confidence ${confidence}/5`, tone: Number(confidence) <= 2 ? 'warning' : 'complete', isComplete, isTrackable: true };
  }
...
function getNextTask(activities: Activity[], statusByActivityId: Record<string, ActivityStatus>, requiredActivityTypes: string[], hasAssignment: boolean) {
  ...
  if (!firstIncomplete) {
    return { label: 'Review pathway', title: hasAssignment ? 'Assignment complete' : 'Pathway complete', detail: 'All required evidence tasks have been saved.', href: '/student/lesson/alexander-ii-reform', button: 'Review route' };
  }

  return { label: activityLabels[firstIncomplete.activity_type] ?? firstIncomplete.activity_type, title: firstIncomplete.title, detail: statusByActivityId[firstIncomplete.id]?.detail ?? 'Open the activity and save your work.', href: activityRouteMap[firstIncomplete.activity_type] ?? '/student/lesson/alexander-ii-reform', button: 'Start now' };
}

export const dynamic = 'force-dynamic';
export const revalidate = 0;

export default async function AlexanderIIReformPathwayPage() {
  if (!supabase) {
    return <main className={styles.shell}><section className={styles.mainCard}><div className={styles.summary}><p className={styles.eyebrow}>Student pathway</p><h1>Supabase is not configured</h1><p>Add the Supabase environment variables in Vercel and redeploy.</p></div></section></main>;
  }
  ...
  if (lessonError || !lesson) {
    return <main className={styles.shell}><section className={styles.mainCard}><div className={styles.summary}><p className={styles.eyebrow}>Student pathway</p><h1>Alexander II reform lesson not found</h1><p>{lessonError?.message ?? 'Add this lesson and its activities in Supabase.'}</p></div></section></main>;
  }
  ...
  const requiredActivityTypes = sortActivityTypesByPathwayOrder(activeAssignment?.required_activity_types?.length ? activeAssignment.required_activity_types : PATHWAY_ACTIVITY_ORDER);
  ...
  const { data: savedResponses } = activityIds.length
    ? await supabase.from('student_responses').select('id, activity_id, response_type, status, score, response_json, submitted_at, last_saved_at').eq('student_id', DEMO_STUDENT_ID).in('activity_id', activityIds)
    : { data: [] };
  ...
  const requiredTrackableStatuses = requiredActivities.map((activity) => statusByActivityId[activity.id]).filter((status) => status?.isTrackable);
  const allTrackableStatuses = orderedActivities.map((activity) => statusByActivityId[activity.id]).filter((status) => status?.isTrackable);
  ...
  return (
    <main className={styles.shell}>
      <div className={styles.topbar}>
        ...
        <div className={styles.topTitle}><span>Guided study route</span><strong>Alexander II reform</strong></div>
      ...
      <section className={styles.mainCard}>
        <header className={styles.summary}>
          ...
          <h1>Alexander II reform</h1>
          <p>{activeAssignment?.instructions ?? 'Complete one task at a time. Focus on why reform became necessary after 1855.'}</p>
        ...
        <section className={styles.nextPanel}>
          <div><p className={styles.eyebrow}>Next task</p><h2>{nextTask.label}</h2><p>{nextTask.title}</p></div>
        ...
        <section className={styles.route}>
          ...
          <div className={styles.progressBar} aria-label="Pathway completion progress"><div className={styles.progressFill} style={{ '--progress': `${progressPercentage}%` } as React.CSSProperties} /></div>
          ...
          <div className={styles.routeList}>
            {requiredActivities.map((activity, index) => {
              const status = statusByActivityId[activity.id];
              const isCurrent = nextTask.href === (activityRouteMap[activity.activity_type] ?? '/student/lesson/alexander-ii-reform') && !status.isComplete;
              return <Link className={`${styles.routeItem} ${status.isComplete ? styles.complete : ''} ${isCurrent ? styles.current : ''}`} href={activityRouteMap[activity.activity_type] ?? '/student/lesson/alexander-ii-reform'} key={activity.id}><span className={styles.routeMark}>{status.isComplete ? '✓' : index + 1}</span><span className={styles.routeText}><strong>{activityLabels[activity.activity_type] ?? activity.title}</strong><span>{activity.title}</span></span><span className={styles.routeStatus}>{status.isComplete ? 'Done' : isCurrent ? 'Next' : status.label}</span></Link>;
            })}
          </div>
          {optionalActivities.length > 0 && <details className={styles.optionalToggle}><summary>Optional support</summary><div className={styles.routeList}>{optionalActivities.map((activity, index) => { const status = statusByActivityId[activity.id]; return <Link className={`${styles.routeItem} ${styles.optional} ${status.isComplete ? styles.complete : ''}`} href={activityRouteMap[activity.activity_type] ?? '/student/lesson/alexander-ii-reform'} key={activity.id}><span className={styles.routeMark}>{status.isComplete ? '✓' : index + 1}</span><span className={styles.routeText}><strong>{activityLabels[activity.activity_type] ?? activity.title}</strong><span>{activity.title}</span></span><span className={styles.routeStatus}>{status.label}</span></Link>; })}</div></details>}
        </section>
      </section>
    </main>
  );
