...
function resolvePathway(slug:string){return activeSubjectPack.pathways.find((item)=>item.pathwaySlug===slug)??getPathwayConfig(slug)}
...
export default async function StudentWorkPage({searchParams}:{searchParams?:Promise<{view?:string}>}){
  ...
  const items=((recipientData??[]) as Recipient[]).flatMap(row=>{const assignment=Array.isArray(row.classroom_assignments)?row.classroom_assignments[0]:row.classroom_assignments;if(!assignment)return[];const progress=progressMap.get(assignment.id);return[{assignment,progress}]})
  ...
  return <main className={styles.page}>
    <header className={styles.header}><div><p className={styles.eyebrow}>My work</p><h1>Assignments</h1><p>Everything your teachers have set, without cluttering your home screen.</p></div></header>
