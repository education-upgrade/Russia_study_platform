...
function attentionReason(assignment: Assignment, progress: Progress | undefined, confidence: number | undefined) {
  ...
  if (notStarted && dueDifference !== null && dueDifference < 0) return 'Overdue and not attempted';
  if (notStarted && dueDifference !== null && dueDifference <= 48 * 60 * 60 * 1000) return 'Not attempted · due soon';
...
export default async function AssignmentWorkspacePage({ params, searchParams }: Props) {
  ...
  const requestedTab = query.tab === 'evidence' || query.tab === 'students' ? 'progress' : query.tab;
  const tab = ['overview', 'progress', 'resources', 'settings'].includes(requestedTab ?? '') ? requestedTab! : 'overview';
  ...
  const filteredRows = rows.filter((row) => {
    ...
    if (filter === 'not_attempted') return assignmentStatus(row.progress) === 'Not attempted';
    if (filter === 'incomplete') return assignmentStatus(row.progress) === 'Incomplete';
    if (filter === 'complete') return assignmentStatus(row.progress) === 'Complete';
    return true;
  });
  const completed = rows.filter((row) => assignmentStatus(row.progress) === 'Complete').length;
  const incomplete = rows.filter((row) => assignmentStatus(row.progress) === 'Incomplete').length;
  const notAttempted = rows.filter((row) => assignmentStatus(row.progress) === 'Not attempted').length;
  ...
  const workspaceTabs = [
    ...
    { id: 'progress', label: 'Progress & evidence' },
    { id: 'resources', label: 'Resources' },
    { id: 'settings', label: 'Settings' },
  ];

  return <main className={styles.workspace}>
    <section className={styles.assignmentHeader}><div><div className={styles.backLink}><Link href="/teacher/assignments">Assignments</Link> <span>›</span> <Link href={`/teacher/classes/${assignment.class_id}?tab=assignments`}>{teachingClass?.name ?? 'Class'}</Link></div><div className={styles.titleRow}><div><p className={styles.eyebrow}>{teachingClass?.name ?? 'Class'} · {teachingClass?.academic_year || 'Academic year not set'}</p><h2>{assignment.title}</h2><p>{assignment.lesson_title} · {assignment.mode.replaceAll('_', ' ')} · Due {formatDate(assignment.due_at)}</p></div><span className={`${styles.statusBadge} ${assignment.status === 'published' ? styles.published : styles.archived}`}>{assignment.status === 'published' ? 'Published' : assignment.status.charAt(0).toUpperCase() + assignment.status.slice(1)}</span></div></div></section>
    <nav className={styles.tabs} aria-label="Assignment workspace">{workspaceTabs.map((item) => <Link key={item.id} href={tabHref(item.id)} className={tab === item.id ? styles.activeTab : undefined} aria-current={tab === item.id ? 'page' : undefined}>{item.label}</Link>)}</nav>

    {tab === 'overview' && <><section className={styles.metrics}><article><span>Complete</span><strong>{completed}/{rows.length}</strong></article><article><span>Average progress</span><strong>{averageProgress}%</strong></article><article><span>Average confidence</span><strong>{averageConfidence}{averageConfidence !== '—' ? '/5' : ''}</strong></article><article><span>Needs attention</span><strong>{attention}</strong></article></section><section className={styles.twoColumn}><article className={styles.card}><header className={styles.sectionHeader}><div><h3>Assignment summary</h3><p>Deadline, route and student instructions.</p></div></header><dl className={styles.summaryList}><div><dt>Deadline</dt><dd>{formatDate(assignment.due_at)}</dd></div><div><dt>Study mode</dt><dd>{assignment.mode.replaceAll('_', ' ')}</dd></div><div><dt>Required activities</dt><dd>{assignment.required_activity_types.length}</dd></div><div><dt>Assigned students</dt><dd>{rows.length}</dd></div></dl><div className={styles.instructions}><span>Student instructions</span><p>{assignment.instructions?.trim() || 'No additional instructions were added.'}</p></div></article><article className={styles.card}><header className={styles.sectionHeader}><div><h3>Current picture</h3><p>Assignment-level completion and intervention signals.</p></div></header><div className={styles.progressTrack}><div style={{ width: `${averageProgress}%` }} /></div><div className={styles.statusGrid}><Link href={`${tabHref('progress')}&filter=not_attempted`}><strong>{notAttempted}</strong><span>Not attempted</span></Link><Link href={`${tabHref('progress')}&filter=incomplete`}><strong>{incomplete}</strong><span>Incomplete</span></Link><Link href={`${tabHref('progress')}&filter=complete`}><strong>{completed}</strong><span>Complete</span></Link><Link href={`${tabHref('progress')}&filter=attention`}><strong>{attention}</strong><span>Attention</span></Link></div></article></section><section className={styles.card}><header className={styles.sectionHeader}><div><h3>Assignment route</h3><p>The activities students complete in sequence.</p></div><span>{assignment.required_activity_types.length} required</span></header><ol className={styles.routeList}>{assignment.required_activity_types.map((activity) => <li key={activity}>{getActivityLabel(activity)}</li>)}</ol></section></>}

    {tab === 'progress' && <section className={styles.card}><header className={styles.sectionHeader}><div><h3>Progress & evidence</h3><p>Review status, confidence and individual saved evidence for this assignment.</p></div><span>{filteredRows.length} shown</span></header><form className={styles.filters}><input type="hidden" name="tab" value="progress" /><input name="q" defaultValue={query.q ?? ''} placeholder="Search students" aria-label="Search students" /><select name="filter" defaultValue={filter} aria-label="Filter students"><option value="all">All students</option><option value="attention">Needs attention</option><option value="not_attempted">Not attempted</option><option value="incomplete">Incomplete</option><option value="complete">Complete</option></select><button type="submit">Apply</button></form><div className={styles.studentList}>{filteredRows.length === 0 ? <div className={styles.empty}><h4>No students match</h4><p>Try changing the search or filter.</p></div> : filteredRows.map((row) => <article className={styles.studentRow} key={row.id}><div><h4>{row.name}</h4><p>{row.reason || `${assignmentStatus(row.progress)} · ${row.progress?.last_activity_at ? `Last active ${formatDate(row.progress.last_activity_at)}` : 'No activity yet'}`}</p><div className={styles.progressTrack}><div style={{ width: `${row.progress?.progress_percent ?? 0}%` }} /></div></div><div className={styles.studentStats}><span><strong>{row.progress?.progress_percent ?? 0}%</strong> progress</span><span><strong>{row.confidence !== undefined ? `${row.confidence}/5` : '—'}</strong> confidence</span><span><strong>{row.progress?.completed_activity_count ?? 0}/{row.progress?.total_activity_count ?? assignment.required_activity_types.length}</strong> activities</span></div><div className={styles.studentActions}><Link className={styles.historyLink} href={`/teacher/classes/${assignment.class_id}/students/${row.id}`}>Student history</Link><Link className={styles.openLink} href={`/teacher/progress/${assignment.id}/${row.id}`}>Open evidence</Link></div></article>)}</div></section>}

    {tab === 'resources' && <section className={styles.card}><header className={styles.sectionHeader}><div><h3>Resources</h3><p>Add lesson-linked support, then choose what students receive with this assignment.</p></div><span>{resources.filter((resource) => resource.attached).length} attached</span></header><LessonResourceManager assignmentId={assignment.id} pathwaySlug={assignment.pathway_slug} resources={resources} /></section>}
    {tab === 'settings' && <section className={styles.settingsGrid}><article className={styles.card}><header className={styles.sectionHeader}><div><h3>Settings</h3><p>Update details, refresh recipients, archive the assignment or delete it permanently.</p></div></header><AssignmentManager assignmentId={assignment.id} instructions={assignment.instructions} dueAt={assignment.due_at} status={assignment.status} /></article><aside className={styles.card}><header className={styles.sectionHeader}><div><h3>What these actions do</h3><p>Changes apply immediately to this assignment.</p></div></header><ul className={styles.guidanceList}><li>Saving changes updates the deadline and student instructions.</li><li>Add new students includes active class members who joined after publication.</li><li>Archiving hides the assignment from student dashboards but keeps evidence.</li><li>Delete permanently removes the assignment and all linked progress and evidence.</li></ul></aside></section>}
  </main>;
}
