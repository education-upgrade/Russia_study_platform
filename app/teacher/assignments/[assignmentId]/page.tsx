...
export default async function AssignmentWorkspacePage({ params, searchParams }: Props) {
  ...
  const requestedTab = query.tab === 'evidence' ? 'students' : query.tab;
  const tab = ['overview', 'students', 'resources', 'settings'].includes(requestedTab ?? '') ? requestedTab! : 'overview';
  ...
  return <main className={styles.workspace}>
    ...
    <nav className={styles.tabs} aria-label="Assignment workspace">{['overview', 'students', 'resources', 'settings'].map((item) => <Link key={item} href={tabHref(item)} className={tab === item ? styles.activeTab : undefined} aria-current={tab === item ? 'page' : undefined}>{item.charAt(0).toUpperCase() + item.slice(1)}</Link>)}</nav>

    {tab === 'overview' && <>
      <section className={styles.metrics}><article><span>Complete</span><strong>{completed}/{rows.length}</strong></article><article><span>Average progress</span><strong>{averageProgress}%</strong></article><article><span>Average confidence</span><strong>{averageConfidence}{averageConfidence !== '—' ? '/5' : ''}</strong></article><article><span>Needs attention</span><strong>{attention}</strong></article></section>
      <section className={styles.twoColumn}>
        <article className={styles.card}><header className={styles.sectionHeader}><div><h3>Assignment summary</h3><p>Deadline, route and student instructions.</p></div></header><dl className={styles.summaryList}><div><dt>Deadline</dt><dd>{formatDate(assignment.due_at)}</dd></div><div><dt>Study mode</dt><dd>{assignment.mode.replaceAll('_', ' ')}</dd></div><div><dt>Required activities</dt><dd>{assignment.required_activity_types.length}</dd></div><div><dt>Assigned students</dt><dd>{rows.length}</dd></div></dl><div className={styles.instructions}><span>Student instructions</span><p>{assignment.instructions?.trim() || 'No additional instructions were added.'}</p></div></article>
        <article className={styles.card}><header className={styles.sectionHeader}><div><h3>Current picture</h3><p>Assignment-level completion and intervention signals.</p></div></header><div className={styles.progressTrack}><div style={{ width: `${averageProgress}%` }} /></div><div className={styles.statusGrid}><Link href={`${tabHref('students')}&filter=not_attempted`}><strong>{notAttempted}</strong><span>Not attempted</span></Link><Link href={`${tabHref('students')}&filter=incomplete`}><strong>{incomplete}</strong><span>Incomplete</span></Link><Link href={`${tabHref('students')}&filter=complete`}><strong>{completed}</strong><span>Complete</span></Link><Link href={`${tabHref('students')}&filter=attention`}><strong>{attention}</strong><span>Attention</span></Link></div></article>
      </section>
    ...
    {tab === 'students' && <section className={styles.card}><header className={styles.sectionHeader}><div><h3>Students</h3><p>One roster for status, confidence and individual evidence.</p></div><span>{filteredRows.length} shown</span></header><form className={styles.filters}><input type="hidden" name="tab" value="students" /><input name="q" defaultValue={query.q ?? ''} placeholder="Search students" aria-label="Search students" /><select name="filter" defaultValue={filter} aria-label="Filter students"><option value="all">All students</option><option value="attention">Needs attention</option><option value="not_attempted">Not attempted</option><option value="incomplete">Incomplete</option><option value="complete">Complete</option></select><button type="submit">Apply</button></form><div className={styles.studentList}>{filteredRows.length === 0 ? <div className={styles.empty}><h4>No students match</h4><p>Try changing the search or filter.</p></div> : filteredRows.map((row) => <article className={styles.studentRow} key={row.id}><div><h4>{row.name}</h4><p>{row.reason || `${assignmentStatus(row.progress)} · ${row.progress?.last_activity_at ? `Last active ${formatDate(row.progress.last_activity_at)}` : 'No activity yet'}`}</p><div className={styles.progressTrack}><div style={{ width: `${row.progress?.progress_percent ?? 0}%` }} /></div></div><div className={styles.studentStats}><span><strong>{row.progress?.progress_percent ?? 0}%</strong> progress</span><span><strong>{row.confidence !== undefined ? `${row.confidence}/5` : '—'}</strong> confidence</span><span><strong>{row.progress?.completed_activity_count ?? 0}/{row.progress?.total_activity_count ?? assignment.required_activity_types.length}</strong> activities</span></div><div className={styles.studentStats}><Link className={styles.openLink} href={`/teacher/classes/${assignment.class_id}/students/${row.id}`}>Student history</Link><Link className={styles.openLink} href={`/teacher/progress/${assignment.id}/${row.id}`}>Open evidence</Link></div></article>)}</div></section>}
