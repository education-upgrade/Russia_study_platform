...
export default async function StudentHistoryPage({ params }: { params: Promise<{ classId: string; studentId: string }> }) {
  ...
  return <main className={styles.page}>
    <section className={styles.header}>
      <div><Link href={`/teacher/classes/${classId}?tab=students`} className={styles.back}>← {teachingClass?.name ?? 'Class'} students</Link><p className={styles.eyebrow}>Student history</p><h1>{name}</h1><p>{teachingClass?.name ?? 'Class'} · {teachingClass?.academic_year || 'Academic year'} · private teacher record</p></div>
