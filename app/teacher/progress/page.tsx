...
const DEMO_CLASSES = [
  { id: DEMO_CLASS_ID, class_name: 'Year 12 Russia demo class', year_group: 'Y12' },
...
function mergeClasses(classData: TeacherClass[]) {
  ...
  return Array.from(classMap.values()).sort((a, b) => a.year_group !== b.year_group ? a.year_group.localeCompare(b.year_group) : a.class_name.localeCompare(b.class_name));
}

function orderActivityTypes(activityTypes: string[]) {
  return [...activityTypes].sort((a, b) => {
    const ai = PATHWAY_ACTIVITY_ORDER.indexOf(a);
    const bi = PATHWAY_ACTIVITY_ORDER.indexOf(b);
...
function buildStudentFeedback(student: { name: string; completed: number; required: number; quizPercentage: number | null; flashcardsSecure: number; flashcardsNearly: number; flashcardsRevisit: number; peelWords: number; confidence: string; confidenceArea: string; flag: string; action: string; }): Feedback {
  ...
  else if (student.quizPercentage !== null) targets.push('Revisit the core 1905 Revolution knowledge from the quiz, especially any questions you missed.');

  if (student.flashcardsSecure >= 8) strengths.push('Your flashcard ratings suggest a strong level of confidence with key terms and events.');
  else if (student.flashcardsSecure > 0) strengths.push('You have started to identify knowledge that feels secure through the flashcards.');
  if (student.flashcardsRevisit > 0) targets.push('Repeat the flashcards marked revisit until they feel secure without support.');
  if (student.flashcardsNearly >= 4) targets.push('Turn your nearly secure flashcards into secure knowledge by testing yourself again.');

  if (student.peelWords >= 100) strengths.push('Your PEEL response shows a fuller attempt to explain historical significance.');
  else if (student.peelWords >= 40) targets.push('Develop your PEEL paragraph by adding more precise evidence and explaining clearly how it weakened Tsarist authority.');
  else targets.push('Complete a fuller PEEL response using Point, Evidence, Explain and Link.');

  const confidenceNumber = Number.parseInt(student.confidence, 10);
  if (Number.isFinite(confidenceNumber) && confidenceNumber >= 4) strengths.push('Your confidence check shows that you feel more secure with this topic.');
  if (student.confidenceArea) targets.push(`Focus your next revision on ${student.confidenceArea}, as this was your least secure area.`);
  if (student.completed < student.required) nextSteps.push('Finish the remaining required activity so your guided study is complete.');
  nextSteps.push(student.action);

  const cleanStrengths = strengths.length ? strengths.slice(0, 3) : ['You have started the 1905 Revolution guided study pathway and evidence is beginning to build.'];
  const cleanTargets = targets.length ? targets.slice(0, 3) : ['Keep consolidating this topic by re-testing yourself and applying precise evidence in written answers.'];
  const cleanNextSteps = unique(nextSteps.filter((step) => step && step !== 'No urgent action.')).slice(0, 2);
...
export default async function TeacherProgressPage({ searchParams }: { searchParams?: Promise<SearchParams> | SearchParams }) {
  ...
  if (!supabase) return <main className={styles.shell}><section className={styles.mainCard}><header className={styles.header}><div><p className={styles.eyebrow}>Teacher progress</p><h1>Supabase is not configured</h1><p>Add the Supabase environment variables in Vercel and redeploy.</p></div></header></section></main>;
  ...
  const membershipStudentIds = memberships.map((m) => m.student_id);
  ...
  const averageProgress = summaries.length ? Math.round(summaries.reduce((total, s) => total + s.progress, 0) / summaries.length) : 0;
  const flaggedStudents = summaries.filter((s) => s.flag !== 'On track');
  const completedStudents = summaries.filter((s) => s.progress === 100).length;
  const averageQuiz = summaries.map((s) => s.quizPercentage).filter((v): v is number => typeof v === 'number');
  const quizAverage = averageQuiz.length ? Math.round(averageQuiz.reduce((t, v) => t + v, 0) / averageQuiz.length) : null;
  const averageConfidence = summaries.map((s) => Number.parseInt(s.confidence, 10)).filter((v) => Number.isFinite(v));
  const confidenceAverage = averageConfidence.length ? (averageConfidence.reduce((t, v) => t + v, 0) / averageConfidence.length).toFixed(1) : null;
  const totalRevisit = summaries.reduce((t, s) => t + s.flashcardsRevisit, 0);
  const filterCounts = { all: summaries.length, missing: summaries.filter((s) => s.hasMissing).length, quiz: summaries.filter((s) => s.quizSupport).length, flashcards: summaries.filter((s) => s.flashcardsSupport).length, peel: summaries.filter((s) => s.peelSupport).length, confidence: summaries.filter((s) => s.confidenceSupport).length };
