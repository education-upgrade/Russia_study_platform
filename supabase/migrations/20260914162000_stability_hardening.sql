-- Stability hardening: keep class joining atomic and make explicit new-attempt retries idempotent.
...
begin
  ...
  on conflict (assignment_id, student_id, activity_type) do update set
    ...
    attempt_count = greatest(
      ...
      when new_attempt_input
       and nullif(excluded.position->>'attemptEventId', '') is not null
       and (public.student_activity_progress.position->>'attemptEventId') is distinct from (excluded.position->>'attemptEventId')
      then 1 else 0
    end,
    score = case
      when public.student_activity_progress.status = 'complete' and excluded.status <> 'complete'
    ...
    position = case
      ...
      else coalesce(public.student_activity_progress.position, '{}'::jsonb) || coalesce(excluded.position, '{}'::jsonb)
    end,
    started_at = coalesce(public.student_activity_progress.started_at, excluded.started_at),
    completed_at = case
      when public.student_activity_progress.status = 'complete' then public.student_activity_progress.completed_at
      when excluded.status = 'complete' then coalesce(public.student_activity_progress.completed_at, timezone('utc', now()))
  ...
  on conflict (assignment_id, student_id) do update set
    ...
    completed_at = case
      when public.assignment_progress.status = 'complete' then public.assignment_progress.completed_at
