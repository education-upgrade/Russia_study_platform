...
using (
  exists (select 1 from public.classes c where c.school_id = schools.id and (public.is_class_teacher(c.id) or public.is_class_student(c.id)))
...
begin
  ...
  if caller_profile.id is null or caller_profile.status <> 'active' or caller_profile.role not in ('teacher', 'admin') then
    raise exception 'Only active teachers can create classes';
  end if;
  if char_length(trim(school_name_input)) < 2 or char_length(trim(class_name_input)) < 2 then
  ...
  insert into public.classes(school_id, name, academic_year, join_code, created_by)
  values (new_school_id, trim(class_name_input), nullif(trim(academic_year_input), ''), public.generate_class_join_code(), auth.uid())
...
begin
  select * into caller_profile from public.profiles where id = auth.uid();
  if caller_profile.id is null or caller_profile.status <> 'active' or caller_profile.role <> 'student' then
