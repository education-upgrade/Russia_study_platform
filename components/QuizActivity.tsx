...
export default function QuizActivity({ activityId, questions, nextHref }: QuizActivityProps) {
      ...
      if (assignmentId) {
        await saveAssignmentActivityProgress({
          ...
          position: {
            ...
            incorrect_question_ids: result.incorrectQuestionIds,
            answers: nextAnswers,
          },
        });
      } else {
  ...
  async function selectAnswer(option: string) {
    if (!currentQuestion || selectedAnswer || completed || saveStatus === 'saving') return;
    const nextAnswers = { ...answers, [currentQuestionKey]: option };
    const isFinalQuestion = currentIndex === questions.length - 1;
    setAnswers(nextAnswers);

    const saved = await saveQuiz(nextAnswers, isFinalQuestion);
    if (!saved) return;
  ...
  function resetQuiz() {
    ...
    if (assignmentId) {
      void saveAssignmentActivityProgress({
        ...
        console.error('Unable to record new quiz attempt', error);
      });
    }
    setAnswers({});
    setCompleted(false);
  ...
  if (completed) {
    return <div className={styles.shell}>
      ...
      <section className={styles.summary}><h2>{score}/{questions.length}</h2><p>{percentage >= 80 ? 'Strong recall. Move on and apply this knowledge.' : 'Good start. Revisit the weaker questions before moving on.'}</p></section>
  ...
  return <div className={styles.shell}>
    ...
    <section className={styles.card}><h2 className={styles.questionText}>{currentQuestion.question}</h2><div className={styles.options}>{currentOptions.map((option) => { const isSelected = selectedAnswer === option; const isCorrect = option === currentQuestion.correct; const showCorrect = Boolean(selectedAnswer) && isCorrect; const showIncorrect = isSelected && !isCorrect; return <button type="button" key={option} onClick={() => void selectAnswer(option)} className={`${styles.option}${isSelected ? ` ${styles.selected}` : ''}${showCorrect ? ` ${styles.correct}` : ''}${showIncorrect ? ` ${styles.incorrect}` : ''}`} disabled={Boolean(selectedAnswer) || saveStatus === 'saving'}>{option}</button>; })}</div></section>
    <section className={styles.bottomNavCompact}>
      <button type="button" className="button secondary" onClick={() => { if (advanceTimerRef.current) clearTimeout(advanceTimerRef.current); setCurrentIndex((previous) => Math.max(previous - 1, 0)); }} disabled={currentIndex === 0}>Previous</button>
      <button type="button" className="button secondary" onClick={() => { if (advanceTimerRef.current) clearTimeout(advanceTimerRef.current); setCurrentIndex((previous) => Math.min(previous + 1, questions.length - 1)); }} disabled={currentIndex === questions.length - 1}>Skip</button>
    </section>
