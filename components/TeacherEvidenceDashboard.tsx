...
export default async function TeacherEvidenceDashboard({ selectedAssignmentId }: Props) {
  ...
  if (!auth || !supabase) {
    return <main className={styles.shell}><section className={styles.mainCard}><header className={styles.header}><h1>Progress is unavailable</h1></header></section></main>;
  }
  ...
  if (assignmentError || !selectedAssignment) {
    return (
      <main className={styles.shell}>
        ...
        <section className={styles.mainCard}><header className={styles.header}><div><p className={styles.eyebrow}>Class overview</p><h1>No published assignments</h1><p>{assignmentError?.message ?? 'Publish an assignment to begin tracking student progress.'}</p></div></header></section>
      </main>
    );
  }
  ...
  const averageProgress = studentRows.length
    ? Math.round(studentRows.reduce((total, row) => total + (row.progress?.progress_percent ?? 0), 0) / studentRows.length)
    : 0;
      ...
      <section className={styles.mainCard}>
        <header className={styles.header}>
          <div>
            ...
            <h1>{teachingClass?.name ?? 'Class progress'}</h1>
            <p>{selectedAssignment.lesson_title} · {formatMode(selectedAssignment.mode)} · Due {formatDate(selectedAssignment.due_at)}</p>
          </div>
          <aside className={styles.decisionCard}>
            <strong>{averageProgress}% average</strong>
            <span>{complete}/{studentRows.length} students complete</span>
          </aside>
        </header>

        {assignments.length > 1 && (
          <section className={styles.priority}>
            <div className={styles.sectionHeader}><h2>Assignments</h2><span className={styles.badge}>{assignments.length} published</span></div>
            <div className={styles.priorityList}>
              {assignments.map((assignment) => {
                const assignmentClass = firstRelation(assignment.teaching_classes);
                return (
                  <article className={styles.priorityItem} key={assignment.id}>
                    <div><strong>{assignment.title}</strong><small>{assignmentClass?.name ?? 'Class'} · {assignment.lesson_title}</small></div>
                    <p>{assignment.assignment_recipients?.[0]?.count ?? 0} students · Due {formatDate(assignment.due_at)}</p>
                    <Link className={styles.navButton} href={`/teacher/progress?assignment=${assignment.id}`}>{assignment.id === selectedAssignment.id ? 'Viewing' : 'View'}</Link>
                  </article>
                );
              })}
            </div>
        ...
        <section className={styles.snapshot}>
          <article className={styles.metric}><span>Not started</span><strong>{notStarted}</strong></article>
          <article className={styles.metric}><span>In progress</span><strong>{inProgress}</strong></article>
          <article className={styles.metric}><span>Complete</span><strong>{complete}</strong></article>
          <article className={styles.metric}><span>Low confidence</span><strong>{lowConfidence}</strong></article>
        </section>
        ...
        <section className={styles.priority}>
          ...
          <div className={styles.tableWrap}>
            <table className={styles.studentTable}>
              <thead><tr><th>Student</th><th>Status</th><th>Progress</th><th>Current task</th><th>Last active</th><th>Confidence</th></tr></thead>
              <tbody>
                {studentRows.length === 0 ? (
                  <tr><td colSpan={6}>No students were assigned this work.</td></tr>
                ) : studentRows.map((row) => (
                  <tr key={row.id}>
                    <td>{row.name}</td>
                    <td><span className={`${styles.statusPill} ${statusStyle(row.progress?.status)}`}>{statusLabel(row.progress?.status)}</span></td>
                    <td>{row.progress?.progress_percent ?? 0}% ({row.progress?.completed_activity_count ?? 0}/{row.progress?.total_activity_count ?? selectedAssignment.required_activity_types.length})</td>
                    <td>{row.progress?.current_activity_type ? getActivityLabel(row.progress.current_activity_type) : '—'}</td>
                    <td>{formatDate(row.progress?.last_activity_at ?? null)}</td>
                    <td>{row.confidence !== undefined ? `${row.confidence}/5` : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section className={styles.studentEvidence}>
          <details className={styles.details}>
            <summary>Assignment route</summary>
            <div className={styles.studentList} style={{ marginTop: 12 }}>
              {selectedAssignment.required_activity_types.map((activityType, index) => (
                <article className={styles.studentCard} key={activityType}>
                  <div className={styles.studentTop}><div><h3>{index + 1}. {getActivityLabel(activityType)}</h3><p>Required activity</p></div></div>
                </article>
              ))}
            </div>
          </details>
