...
const balancedAtAssembly=new Set(['k-23','a2-57','a3-41','s-81']);
for(const source of [expanded,depth])for(const line of source.split('\n').filter(line=>line.trim().startsWith('mcq('))){const id=line.match(/mcq\('([^']+)'/)?.[1];const optionMatch=line.match(/\[(.*?)\],(\d),/);if(!optionMatch){console.error(`Could not audit MCQ: ${line.slice(0,90)}`);process.exit(1);}const options=[...optionMatch[1].matchAll(/'([^']*)'/g)].map(m=>m[1]);if(options.length!==4){console.error(`MCQ ${id} has ${options.length} options instead of 4.`);process.exit(1);}const correct=Number(optionMatch[2]);const lengths=options.map(option=>option.length);const longest=Math.max(...lengths);const second=[...lengths].sort((a,b)=>b-a)[1];if(!balancedAtAssembly.has(id??'')&&lengths[correct]===longest&&longest-second>=24){console.error(`MCQ ${id} gives away the answer by length.`);process.exit(1);}}
